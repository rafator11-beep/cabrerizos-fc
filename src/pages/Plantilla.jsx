...
export default function Plantilla() {
  ...
  const [pizarraZoom, setPizarraZoom] = useState(1);
        ...
        <div className="fixed inset-0 z-[120] bg-bg flex flex-col animate-in fade-in duration-300">
          ...
          <div className="flex-1 flex flex-col min-h-0 md:flex-row overflow-hidden">
            ...
            <div className="flex-1 min-h-0 bg-surface-2 relative group flex flex-col">
              ...
              {/* Quick Elements Bar */}
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4 px-6 py-3 bg-black/40 backdrop-blur-2xl rounded-full border border-white/10 shadow-2xl z-10">
  ...
  return (
    <div className="card" style={{ padding: 12, position: 'relative' }}>
      ...
      <div onClick={isAdmin ? onPhotoClick : undefined}
          className="bg-transparent" style={{ width: 52, height: 52, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 800, color: '#00ff87', overflow: 'visible', border: 'none', cursor: isAdmin ? 'pointer' : 'default', position: 'relative', flexShrink: 0 }}>
          {p.photo_url ? (
            <img src={p.photo_url} alt="" className="bg-transparent object-contain drop-shadow-lg" style={{ 
              width: '100%', 
              height: '100%'
            }} />
          ...
          {uploading && <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,.6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ width: 14, height: 14, border: '2px solid white', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 1s linear infinite' }} /></div>}
        </div>
