        ...
        <div style={{ background: '#f8f9fb', padding: 8, borderRadius: 8, marginBottom: 6 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6, maxHeight: 200, overflowY: 'auto' }}>
            {filteredExercises.map(exData => (
              <div key={exData.id} style={{ position: 'relative', cursor: 'pointer' }} onClick={async () => {
                const drawing = exData.canvas_drawing || null;
                setExerciseInput(ei => ({
                  ...ei,
                  image: exData.image,
                  name: exData.name,
                  description: exData.description || '',
                  duration: exData.duration || 15,
                  category: exData.category,
                  meta: exData.meta,
                  canvas_drawing: drawing,
                }));
                if (drawing) {
                  setPzTokens(drawing.tokens || []);
                  setPzArrows(drawing.arrows || []);
                  setPzZones(drawing.zones || []);
                  setShowGallery(false);
                  setShowPizarra(true);
                  return;
                }
                // Busca en Supabase por nombre de ejercicio en jugadas guardadas
                try {
                  const { data } = await supabase
                    ...
                    .limit(50);
                  for (const t of data || []) {
                    const match = (t.exercises || []).find(e => e.name === exData.name && e.canvas_drawing);
                    if (match) {
                      setPzTokens(match.canvas_drawing.tokens || []);
                      setPzArrows(match.canvas_drawing.arrows || []);
                      setPzZones(match.canvas_drawing.zones || []);
                      setExerciseInput(ei => ({ ...ei, canvas_drawing: match.canvas_drawing }));
                      setShowGallery(false);
                      setShowPizarra(true);
                      return;
                    }
                  }
                } catch { /* sin dibujo guardado */ }
                setShowGallery(false);
              }}>
                <img
