...
export default function Alineacion() {
  ...
  const saveLineup = async () => {
    ...
    try {
      ...
      alert('âœ… AlineaciÃ³n guardada y notificaciones enviadas a los jugadores');
    } catch (error) {
      alert('âŒ Error al guardar la alineaciÃ³n');
    }
  };
  ...
  const sendLineupNotifications = async () => {
    ...
    const notification = {
      title: `Nueva AlineaciÃ³n: ${activeLineup.name}`,
      message: `Has sido convocado para el partido. Revisa tu posiciÃ³n y comentarios del entrenador.`,
      type: 'lineup',
      lineup_id: activeLineup.id,
      created_at: new Date().toISOString()
    };

    // Insertar notificaciÃ³n para cada jugador convocado
    ...
    <div className="flex flex-col h-full bg-bg overflow-hidden md:flex-row">
          ...
          <div className="modal-sheet" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted">Instrucciones especÃ­ficas para el partido</p>
              </div>
            </div>
            <textarea 
              ...
              placeholder="Ej: Mantente pegado a la banda, busca el desmarque por detrÃ¡s de la defensa..."
              value={playerComment}
      ...
      {showForm && (
        <div className="modal-overlay animate-fade-in" onClick={() => setShowForm(false)}>
          <div className="modal-sheet" onClick={e => e.stopPropagation()}>
            <h2 className="text-xl font-black text-white mb-6">Nueva AlineaciÃ³n</h2>
            <input className="input-field mb-3" placeholder="Ej: Jornada 15 vs BÃ©jar" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} autoFocus />
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-3 space-y-2">
          {lineups.map(l => (
            <button key={l.id} onClick={() => { setActiveLineup(l); if(isMobile) setMobileTab('field'); }}
              ...
              <div className="text-[9px] font-bold mt-0.5 opacity-40">{l.match_date} Â· {l.formation}</div>
            </button>
          ))}
        </div>
      </div>
                      ...
                      <>
                        ...
                        {isCaptain && (
                          <g transform="translate(0, -35)">
                            ...
                            <text textAnchor="middle" dy="3" fontSize="8" fill="white">ðŸ‘‘</text>
                          </g>
                        )}
                        ...
                        {hasComments && (
                          <g transform="translate(15, -15)">
                            ...
                            <text textAnchor="middle" dy="2" fontSize="6" fill="white">ðŸ’¬</text>
                          </g>
                        )}
                        ...
                        {!isPlayerMode && p && (
                          <g transform="translate(20, 0)">
                            ...
                            <text textAnchor="middle" dy="3" fontSize="8" fill="white" className="cursor-pointer pointer-events-none">ðŸ‘‘</text>
