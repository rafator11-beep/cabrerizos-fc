...
import { Home, PenTool, Dumbbell, Target, Users, LayoutGrid, MessageSquare, X, CalendarCheck, ShieldCheck, User, Clock, Trophy } from 'lucide-react';
...
export default function Sidebar({ onClose, isMobile }) {
  ...
  const NAV = [
    { id: '', icon: <Home size={18} />, label: 'Inicio', show: true, section: null },
    { id: 'mi-sesion', icon: <CalendarCheck size={18} />, label: 'Mi Sesión', show: !isAdmin, section: null },
    { id: 'entrenamientos', icon: <Dumbbell size={18} />, label: 'Entrenamientos', show: true, section: 'GESTIÓN' },
    { id: 'tactica', icon: <PenTool size={18} />, label: 'Táctica', show: true, section: null },
    { id: 'tecnica', icon: <Target size={18} />, label: 'Técnica', show: true, section: null },
    { id: 'plantilla', icon: <Users size={18} />, label: 'Plantilla', show: true, section: 'EQUIPO' },
    { id: 'alineacion', icon: <LayoutGrid size={18} />, label: 'Alineación', show: true, section: null },
    { id: 'feedback', icon: <MessageSquare size={18} />, label: 'Feedback', show: true, section: 'COMUNIDAD' },
  ];
        ...
        <nav className="px-3 space-y-0.5">
          {NAV.filter(item => item.show).map(item => {
            ...
            return (
              <div key={item.id}>
                {showSection && (
                  <div className="px-4 pt-5 pb-2 text-[9px] font-black text-muted/30 uppercase tracking-[0.25em]">
                    {item.section}
                  </div>
                )}
                <NavLink
                  ...
                  className={({ isActive }) => `
                    flex items-center gap-3 px-4 py-2.5 rounded-xl font-bold text-[13px] transition-all duration-200
                    ${isActive 
                      ? 'bg-accent/10 text-accent shadow-[inset_0_0_20px_rgba(59,130,246,0.05)] border-l-2 border-accent ml-0.5' 
