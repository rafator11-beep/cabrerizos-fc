// FUNCIONES COMPLETAS PARA AÑADIR A ALINEACION.JSX
// Añadir después de la función sendLineupNotifications
...
const LineupContextMenu = ({ contextMenu, onEdit, onDelete, onDuplicate, onClose }) => {
  if (!contextMenu) return null;

  return (
    <div 
      className="fixed bg-surface border border-border rounded-lg shadow-xl z-50 py-2 min-w-[160px]"
      style={{ 
        left: contextMenu.x, 
        top: contextMenu.y,
    ...
    >
      <button
        onClick={() => { onEdit(contextMenu.lineup); onClose(); }}
        className="w-full px-4 py-2 text-left text-sm hover:bg-surface-2 flex items-center gap-2 text-text"
      >
        <Edit3 size={14} />
        Editar
      </button>
      <button
        onClick={() => { onDuplicate(contextMenu.lineup); onClose(); }}
        className="w-full px-4 py-2 text-left text-sm hover:bg-surface-2 flex items-center gap-2 text-text"
      >
        <Copy size={14} />
        Duplicar
      ...
      <button
        onClick={() => { onDelete(contextMenu.lineup.id); onClose(); }}
